...
Модель (sklearn Pipeline текст → класс) грузится из MODEL_PATH. Если файла нет или он
несовместим — работает прозрачная эвристика-заглушка, и сервис остаётся полностью рабочим.
Замена модели не затрагивает остальной сервис (API, витрину).
"""
import logging
import os
