...
class Settings(BaseSettings):
    ...
    # Дотяг полного текста для пачки новых статей идёт параллельно, чтобы всплеск новинок
    # не растягивал цикл сбора (важно при коротком INGEST_INTERVAL_MINUTES).
    FULLTEXT_WORKERS: int = 8    # число параллельных загрузок полного текста
    FULLTEXT_TIMEOUT: int = 15   # таймаут одной загрузки, сек (ограничивает длину цикла)

    # Коннектор ВК (паблики). Сервисный токен приложения dev.vk.com; пусто -> vk-источники пропускаются.
    VK_TOKEN: str = ""
    VK_API_VERSION: str = "5.199"
    VK_POST_COUNT: int = 50
